...
def checkLine(vals):
    vals = sorted(map(int, vals), reverse = True)
    valLen = len(vals)
    for curValIndex in range(0, valLen):
        curVal = int(vals[curValIndex]);
        for compValIndex in range(curValIndex + 1, valLen):
            compVal = int(vals[compValIndex])
            if (curVal % compVal == 0):
